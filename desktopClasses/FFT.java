 ...
 * </p>
 * <p>
 * The FFT window size is determined by the size of the input sample array. FFT windows size: the
 * input sample array length rounded up to the next higher power of 2.
 * </p>
 *
...
public class FFT {
  ...
  private DoubleFFT_1D fft_1D_Double;
  ...
  /**
   * Creates an instance of {@code FFT} with the specified resolution and a Hamming window by
   * default.
   * <p>
  ...
  /**
   * public FFT(@NonNull WindowType windowType) { this.windowType = windowType; initialise(); }
   * 
   **/
  ...
  /**
   * public FFT(int fftResolution, WindowType windowType) { this(fftResolution); this.windowType =
   * windowType; }
   **/
   ...
   * @throws IllegalArgumentException - if samples block contains two channels but the block length
   *         is odd - if channels < 1 or channels > 2
   */
  public float[] getPowerSpectrum(short[] samples, int channels) {
    ...
    if (samples.length > windowSize) {
      /*
       * We can't process the samples because the FFT resolution is too small. One solution would be
       * to cut sample block into smaller pieces and process FFT. However, it is better to increase
       * the fft resolution for better accuracy in the frequency domain as this is the goal for this
       * app.
       */
      return new float[0];
    }
  ...
  /**
   * <p>
   * Computes the FFT of real input data. Sample input data is filtered with the specified window.
   * Samples are automatically zero-padded if the sample size is not a power of 2. Returns only
   * first half of DFT spectrum, second half is ignored because of symmetry.
   * </p>
   *
  ...
  /**
   * <p>
   * Computes the FFT of real float input data. No window gets applied Samples are automatically
   * zero-padded if the sample size is not a power of 2. Returns only first half of DFT spectrum,
   * second half is ignored because of symmetry.
   * </p>
   *
   * @param samples the sample block to transform
   * @return the transformed data, input remains unchanged
   */
  public float[] getForwardTransformNoWindow(float[] samples) {
    if (sampleSize != samples.length) {
      // onSampleSizeChanged(samples.length);
    }

    float[] copyOfSamples = new float[samples.length];
    System.arraycopy(samples, 0, copyOfSamples, 0, samples.length);


    // Zero-padding if necessary
    int paddingLength = getPaddingLength(sampleSize);
    if (paddingLength > 0) {
      copyOfSamples = applyZeroPaddingToSamples(copyOfSamples, paddingLength);
    }

    FloatFFT_1D fft = new FloatFFT_1D(copyOfSamples.length);
    fft.realForward(copyOfSamples);
    return copyOfSamples;
  }

  /**
   * <p>
   * Computes the FFT of real double input data. No window gets applied Samples are automatically
   * zero-padded if the sample size is not a power of 2. Returns only first half of DFT spectrum,
   * second half is ignored because of symmetry.
   * </p>
   *
   ...
   * @return the transformed data, input remains unchanged
   */
  public double[] getForwardTransformNoWindow(double[] samples) {
    if (sampleSize != samples.length) {
      // onSampleSizeChanged(samples.length);
    }

    double[] copyOfSamples = new double[samples.length];
    System.arraycopy(samples, 0, copyOfSamples, 0, samples.length);
    ...
    DoubleFFT_1D fft = new DoubleFFT_1D(copyOfSamples.length);
    fft.realForward(copyOfSamples);
    return copyOfSamples;
  }
   ...
   * <p>
   * Computes the FFT of real input data. Sample input data is filtered with the specified window.
   * Samples are automatically zero-padded if the sample size is not a power of 2. Returns the full
   * DFT spectrum.
   * </p>
   *
   ...
   *
   * @param samples an array of {@code float}
   * @return the transformed data in an array of {@code double}
   */
  public double[] getForwardTransformFull(double[] samples) {
    ...
    double[] weightedSamples = new double[samples.length];
    System.arraycopy(samples, 0, weightedSamples, 0, samples.length);

    int paddingLength = getPaddingLength(sampleSize);
    if (paddingLength > 0) {
      weightedSamples = applyZeroPaddingToSamples(samples, paddingLength);
    }
    DoubleFFT_1D fft = new DoubleFFT_1D(weightedSamples.length);
  ...
  /**
   * Computes real fft with given float fftSamples, does not check or correct valid padding
   * 
   * @param fftSamples
   */
  public float[] getIFFT(float[] fftSamples) {

    float[] copy = new float[fftSamples.length];
    System.arraycopy(fftSamples, 0, copy, 0, fftSamples.length);

    FloatFFT_1D fft = new FloatFFT_1D(copy.length);
    fft.realInverse(copy, false);
    return copy;
  }

  /**
   * Computes real fft with given double fftSamples, does not check or correct valid padding
   * 
   * @param fftSamples
   */
  public double[] getIFFT(double[] fftSamples) {

    double[] copy = new double[fftSamples.length];
    System.arraycopy(fftSamples, 0, copy, 0, fftSamples.length);

    DoubleFFT_1D fft = new DoubleFFT_1D(copy.length);
    fft.realInverse(copy, false);
    return copy;
  }

  public double[] getIFFTComplex(double[] fftSamples) {
    double[] copy = new double[fftSamples.length];
    System.arraycopy(fftSamples, 0, copy, 0, fftSamples.length);

    DoubleFFT_1D fft = new DoubleFFT_1D(copy.length);
    fft.complexInverse(copy, false);
    return copy;
  }
  ...
  /**
   * Adds zero padding of specified length to a sample block of type float.
   *
  ...
  /**
   * Adds zero padding of specified length to a sample block of type double.
   *
   * @param samples the input sample block
   * @param paddingLength the padding length
   * @return zero padded sample block
   * @throws IllegalArgumentException if paddingLength less than 0
   */
  private double[] applyZeroPaddingToSamples(double[] samples, int paddingLength)
      throws IllegalArgumentException {
    if (paddingLength < 0) {
      throw new IllegalArgumentException("Illegal padding length: " + paddingLength);
    }
  ...
  /**
   * private void onSampleSizeChanged(int sampleSize) { this.sampleSize = sampleSize;
   * createWindow();
   * 
   * }
   **/
  ...
  /**
   * private void createWindow() { if (sampleSize > 0) { window = new
   * Window(windowType).getWindow(sampleSize); } else { window = null; } }
   * 
   **/
}
