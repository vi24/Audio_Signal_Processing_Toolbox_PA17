  ...
  public short[] getNextSampleBlock() {
    ...
    try {
      ...
      if (currentFrameHeader != null) {
        ...
        if (channels == MONO) {
          int length = sampleBlock.length;
          int newLength = length / 2;
          short[] tempSampleBlock = new short[newLength];
          System.arraycopy(sampleBlock, 0, tempSampleBlock, 0, newLength);
          sampleBlock = tempSampleBlock;
        } /**else if (channels == STEREO) {
          short[] tempSampleBlock = new short[sampleBlock.length / 2];
          int counter = 0;
          for (int i = 0; i < sampleBlock.length; i++) {
            if (i % 2 == 0) {
              tempSampleBlock[counter] = sampleBlock[i];
              counter++;
            }
          }
          sampleBlock = tempSampleBlock;
