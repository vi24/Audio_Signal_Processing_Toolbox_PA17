function [b,onsetenv,oesr,D,cumscore,localpd] = beatdyn(d,sr,startbpm,tightness,doplot)
...
localpd  = startpd * ones(1,length(localscore));
...
for i = 1:length(localscore)
  ...
  % weighting
  scorecands = (-tightness*abs((log(-prange./[startpd*ones(1,zpad),localpd(timerange(zpad+1:end))])).^2)) + [zeros(1,zpad),cumscore(timerange(zpad+1:end))];
...
%  if starting == 1 & localscore(i) > 100*abs(vv)
  ...
  if backlink(i) > 0
    % local period
    localpd(i) = ita * (i-backlink(i)) + (1-ita)*localpd(backlink(i));
  end
...
% return dynamic tempo period estimates at those beats
localpd = localpd(b);
